...
public class MainHubUIManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // 메인 행동 버튼 리스너 설정
        pauseButton.onClick.AddListener(OpenPauseMenuPanel);
        ...
        // 일시정지 패널 버튼 리스너 설정
        resumeButton.onClick.AddListener(ClosePauseMenuPanel);
    ...
    private void OpenPauseMenuPanel()
    {
    ...
    private void EnterTower()
    {
        ...
        Debug.Log("탑 탐험 시작!");
        SceneManager.LoadScene("EnterTowerScene");
    }
    ...
    private void OpenConfigPanel()
    {
        if(pauseMenuPanel != null)
        {
            pauseMenuPanel.SetActive(false); // 일시정지 패널 비활성화
        }
